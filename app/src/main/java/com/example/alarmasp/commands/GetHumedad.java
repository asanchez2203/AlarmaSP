...
public class GetHumedad extends AsyncTask<Object,Integer,Integer> {
    ...
    public GetHumedad(Context context, TextView edLightIntensity){
        progressDialog = new ModalProgressDialog(context,"Verificando la Humedad",
    ...
    protected Integer doInBackground(Object... objects) {
        ...
        if(cnEnv!=null){
            ...
            try {
                ...
                st.executeUpdate("INSERT INTO Table_Request (typeRequest,valueReq,attended) VALUES ("+ TypeRequest.GET_HUMIDITY +",'',0)",
                        Statement.RETURN_GENERATED_KEYS);
                ResultSet keyGenerated = st.getGeneratedKeys();
                keyGenerated.next();
                int idRequest = keyGenerated.getInt(1);
                Log.println(Log.INFO,"Humedad","Request Number: "+ idRequest);
                st.close();
                ...
                do {
                    ...
                    attemp++;
                    Thread.sleep(2000);
                    Log.println(Log.INFO,"Humedad","Value: "+ response);
                }while(response ==null);
                st.close();
                if(response!=null || !response.equals("None")){
                    lightIntensity = (int)Double.parseDouble(response);
                }else lightIntensity = -1;
                Log.println(Log.INFO,"Humedad","Final value: "+ response);

                cnEnv.close();
                cn.close();
                return lightIntensity;
            } catch (Exception throwables) {
                Log.println(Log.ERROR,"Fail Get Humedad",
    ...
    protected void onPostExecute(Integer integer) {
        modalDialog.setTitle("Humedad");
        progressDialog.hideProgressDialog();
        if(lightIntensity == -1) {
            modalDialog.setMessage("Hubo un error al obtener la Humedad");
            modalDialog.showModalDialog();
            tvTemp.setText("None");
        }else{
            tvTemp.setText(String.valueOf(lightIntensity));
        }
    }
}
